...
const textNeedHeight = (paragraphs) => {
    return (paragraphs.length - 1) * opt.paragraphsMarginBottom + paragraphs.flat().length * opt.lineHeight * opt.fontSize
}
/**
...
async function get_tweet_card(tp, input) {
    ...
    opt.cardHeight = opt.contentHeight + opt.paddingTB * 2 + opt.logoSize + opt.lineHeight * opt.fontSize /** 用来书写时间 */ + 2 * opt.paragraphsMarginBottom
    /** 放在内容上下 */
